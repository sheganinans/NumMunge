...
These dependencies are fetched from [Hackage](https://hackage.haskell.org/), Haskell's store for packages. Next up is a `library` stanza. It defines where the source is located, in our case `src/`, and what modules can be found there. In our case that's just a single module, `NumMunge.Project`.
...
Note: If you need a newer Clash version, simply change the version bounds in `nm-clash.cabal` and follow the hints given by Stack.

## src/

This is where the source code of the project lives, as specified in `nm-clash.cabal`. It contains a single file, `NumMunge/Project.hs` which starts with:

```haskell
module NumMunge.Project (topEntity, plus) where
...
`nm-clash.cabal` enabled `NoImplicitPrelude` which enables the use of `Clash.Prelude` here. Next, a function `plus` is defined. It simply adds two numbers. Note that the example (`>>> plus 3 5`) gets executed by the _doctests_ defined for this project and checked for consistency with the result in the documentation (`8`).

```haskell
-- | 'topEntity' is Clash's equivalent of 'main' in other programming
-- languages. Clash will look for it when compiling 'NumMunge.Project'
...
```
stack run clash -- NumMunge.Project --vhdl -main-is plus
```
...
Most of this directory is scaffolding, with the meat of the tests being defined in `tests/Tests/NumMunge/Project.hs`. Writing good test cases is pretty hard: edge cases are easy to forget both in the implementation and tests. To this end, it's a good idea to use _fuzz testing_. In this project we use [Hedgehog](https://hedgehog.qa/):

```haskell
import NumMunge.Project (plus)
...
By default `nm-clash.cabal` sets its `license` field to `BSD-2-Clause`. You might want to change this.
